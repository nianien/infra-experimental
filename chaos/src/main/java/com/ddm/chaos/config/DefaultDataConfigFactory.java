 ...
 *   <li>类型转换：使用 {@code ConcurrentHashMap.computeIfAbsent()} 保证每个 &lt;key,provider&gt; 仅转换一次</li>
 * </ul>
 *
 ...
 *
 * @author liyifei
 * @see DataConfigFactory
 * @since 1.0
 */
public final class DefaultDataConfigFactory implements DataConfigFactory {

    private static final Logger log = LoggerFactory.getLogger(DefaultDataConfigFactory.class);
    ...
    private final DataConfigProperties props;
     ...
     * <p>特点：
     * <ul>
     *   <li>Key：{@code TypedKey(key, provider)}，由配置键和目标类型组成</li>
     ...
     */
    public DefaultDataConfigFactory(DataConfigProperties props) throws Exception {
        this.props = props;
        this.init();
    }
    ...
    public synchronized void startRefresh() {
        ...
        if (refreshIntervalMs > 0) {
            ...
            scheduler = Executors.newSingleThreadScheduledExecutor(r -> {
                Thread t = new Thread(r, "chaos-data-refresh");
                t.setDaemon(true);
     ...
     * <p>转换流程：
     * <ol>
     *   <li>检查 typedCache 中是否存在该 &lt;key,provider&gt; 的缓存</li>
     ...
     * @throws NullPointerException 如果 key 或 provider 为 null
     */
    @Override
    @SuppressWarnings("unchecked")
    public <T> Supplier<T> getSupplier(TypedKey key) {
        Objects.requireNonNull(key, "key cannot be null");
        return () -> {
            // 使用 computeIfAbsent 保证原子性和唯一转换
            Object v = typedCache.computeIfAbsent(key, tk -> {
                // 从原始缓存中获取值
                Object raw = rawCache.get(tk.name());
                try {
                    // 进行类型转换
                    Object converted = cast(raw == null ? tk.value() : raw, tk.type());
                    // 如果转换结果为 null，使用 NULL 哨兵
                    return (converted == null) ? NULL : converted;
                } catch (Exception ex) {
                    // 转换失败，记录日志并使用 NULL 哨兵（避免后续反复抛出异常）
                    log.debug("Type convert failed for key='{}', provider='{}': {}",
                            key, tk.type().getSimpleName(), ex.getMessage());
                    return NULL;
                }
            });
            // 将 NULL 哨兵转换回 null
    ...
    public void refreshAll() {
        ...
        try {
            ...
            log.debug("Configuration refreshed successfully: {} entries", rawCache.size());
    ...
    private static DataProvider loadDataProvider(String type) {
        ...
        throw new IllegalStateException("No DataProvider found via SPI for: " + type);
    }
}
