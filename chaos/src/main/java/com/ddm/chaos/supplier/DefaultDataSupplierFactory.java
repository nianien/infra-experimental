...
public final class DefaultDataSupplierFactory implements DataSupplierFactory {
    ...
    public void setRefreshInterval(Duration duration) {
        Objects.requireNonNull(duration, "refreshInterval cannot be null");
        this.refreshInterval = duration.isNegative() ? Duration.ZERO : duration;
    }
        ...
        return () -> {
            ...
            Object v = typedCache.computeIfAbsent(new TypedKey(key, type), tk -> {
                ...
                try {
                    ...
                    log.warn("Type conversion failed for key={}, type={}",
                            key, type.getName(), ex);
                    return NULL;
                }
            });
    ...
    private void refreshAll() {
        ...
        try {
            ...
            // 刷新失败时保留旧镜像，保证系统可用性
            log.warn("Refresh failed, keeping previous cache (will retry on next interval): {}",
                    e.getMessage(), e);
        } finally {
            refreshLock.unlock();
        }
